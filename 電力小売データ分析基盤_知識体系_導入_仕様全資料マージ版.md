...
**生成日**：2026-09-10
**目的**：レビュー用および今後のナレッジ入力用に、設計仕様書・詳細設計書・全SQL・運用スクリプト・教育資料を1ファイルへ統合したもの。
**元構成**：`power_retail_data_platform_repo`（設計・実装）と `power_retail_data_platform_guide`（教育資料）

> 本ファイルは各原本を無加工で収録し、見出しレベルのみ繰り下げている。**正本は各原本ファイル**であり、修正は原本側に対して行う。
> 数値（単価・損失率・制度仕様）は設計イメージを含む。実装前に一次情報（託送供給等約款、JEPX規程、OCCTO・資源エネルギー庁の公表資料）で検証すること。

## 収録一覧

| 部 | 収録内容 | 件数 |
|---|---|---|
| 第1部　設計仕様書（正本） | 設計仕様書 | 1 |
| 第2部　詳細設計書・定義書 | 詳細設計書／損益分析・利益階層 定義書／BigQuery 命名・運用定義書 適合チェック結果／SQL リポジトリ構成（README） | 4 |
| 第3部　DDL（テーブル定義） | Silver 層 マスタ（dim_*）／Silver 層 ファクト（fact_*）／Gold 層 マート（agg_* / snap_*）／契約履歴 SCD Type 2 検証SQL | 4 |
| 第4部　ユーザー定義関数（UDF） | 地点番号 先頭3桁検証 UDF | 1 |
| 第5部　ビュー | 料金用休日ビュー／需要実績 時系列統合ビュー／発電実績 時系列統合ビュー／実量kW判定ビュー／市場連動単価解決ビュー／利益階層 日次ビュー／BI抽出用 日報ビュー | 7 |
| 第6部　ストアドプロシージャ | 日報損益マート生成／日次検算（#1〜#3）／コンソーシアムBG按分／需要予測精度マート生成／祝日CSV取込（フェイルセーフ付き）／訪問検針プロファイル配分／契約移行ゲート（SCD2検証）／BI抽出完了監視 | 8 |
| 第7部　運用スクリプト | コマ別集約マート アーカイブ（Active→Cold） | 1 |
| 第8部　教育資料（導入ガイド） | 導入ガイド README（読む順序・役割別経路）／知識体系：電力事業データ分析基盤 ナレッジ体系／初級・入門編：業界基礎知識講義／中級：数字ができるまでを1日追う／上級：設計判断・運用統制・事業経済性／応用編：実務の落とし穴と設計判断 | 6 |
...
| ファイル | 除外理由 |
|---|---|
| `guide_en/99_combined/00_combined_guide.md` | 第8部の5資料を1本に結合しただけの版。内容が重複するため除外 |
| `repo_en/sql/ALL_IN_ONE.sql` | 第3〜6部の SQL をデプロイ順に結合しただけの版。内容が重複するため除外 |

**読み方**

- **レビュー目的**：第1部（設計仕様書）→ 第3〜6部（実装SQL）の順に読み、仕様と実装の対応を確認する。第2部の適合チェック結果に未解決事項がまとまっている。
- **ナレッジ入力目的**：第8部（教育資料）が業務知識と設計判断の背景を、第1〜2部が確定仕様を、第3〜7部が実装を持つ。
- **オブジェクト命名**：`dim_`（マスタ）／`fact_`（ファクト）／`agg_`（集計マート）／`snap_`（スナップショット）／`v_`（ビュー）／`udf_`（関数）／`p_`（プロシージャ）。Bronze層はデータセット `bronze_*` で識別し接頭辞を持たない。
- **テンプレート変数**：SQL 中の `${GCP_PROJECT_ID}`／`${BQ_DATASET_*}` は CI/CD で環境別の値へ展開してから実行する。未展開のまま実行してはならない。
...
> **原本**：`repo_en/docs/design_detail.md`　／　1143 行
...
> **原本**：`repo_en/docs/profit_layer_definitions.md`　／　215 行
...
## BigQuery 命名・運用定義書 適合チェック結果

> **原本**：`repo_en/docs/sql_standard_compliance_report.md`　／　205 行
...
> **原本**：`repo_en/sql/ddl/masters.sql`　／　690 行
...
> **原本**：`repo_en/sql/ddl/facts.sql`　／　599 行
...
## Gold 層 マート（agg_* / snap_*）

> **原本**：`repo_en/sql/ddl/marts.sql`　／　274 行
...
## 契約履歴 SCD Type 2 検証SQL

> **原本**：`repo_en/sql/ddl/scd2_validation_customer_contracts.sql`　／　42 行
...
> **原本**：`repo_en/sql/function/udf_validate_demand_point_number.sql`　／　38 行
...
> **原本**：`repo_en/sql/view/v_rate_holiday_priority.sql`　／　24 行
...
> **原本**：`repo_en/sql/view/v_dem_actuals_timeline.sql`　／　44 行
...
> **原本**：`repo_en/sql/view/v_gen_actuals_timeline.sql`　／　41 行
...
> **原本**：`repo_en/sql/view/v_actual_peak_kw_resolver.sql`　／　82 行
...
> **原本**：`repo_en/sql/view/v_market_linked_price_resolver.sql`　／　70 行
...
> **原本**：`repo_en/sql/view/v_profit_layers_daily.sql`　／　21 行
...
> **原本**：`repo_en/sql/view/v_bi_daily_pnl_extract.sql`　／　33 行
...
> **原本**：`repo_en/sql/procedure/p_generate_daily_pnl.sql`　／　540 行
...
> **原本**：`repo_en/sql/procedure/p_verify_daily_pnl.sql`　／　80 行
...
> **原本**：`repo_en/sql/procedure/p_calculate_bg_member_imbalance.sql`　／　145 行
...
> **原本**：`repo_en/sql/procedure/p_generate_forecast_accuracy_daily.sql`　／　82 行
...
> **原本**：`repo_en/sql/procedure/p_load_public_holidays.sql`　／　104 行
...
> **原本**：`repo_en/sql/procedure/p_execute_load_profile_allocation.sql`　／　79 行
...
> **原本**：`repo_en/sql/procedure/p_validate_and_gate_customer_contracts.sql`　／　75 行
...
> **原本**：`repo_en/sql/procedure/p_monitor_bi_extract_sync.sql`　／　61 行
...
## コマ別集約マート アーカイブ（Active→Cold）

> **原本**：`repo_en/ops/b13_archive_slot_summary.sh`　／　17 行
...
## 導入ガイド README（読む順序・役割別経路）

> **原本**：`guide_en/README.md`　／　113 行
...
## 知識体系：電力事業データ分析基盤 ナレッジ体系

> **原本**：`guide_en/01_knowledge_system/01_knowledge_system.md`　／　748 行
...
> **原本**：`guide_en/02_beginner/02_beginner_industry_basics.md`　／　424 行
...
> **原本**：`guide_en/03_intermediate/03_intermediate_one_day_walkthrough.md`　／　291 行
...
> **原本**：`guide_en/04_advanced/04_advanced_design_and_economics.md`　／　206 行
...
> **原本**：`guide_en/05_applied/05_applied_pitfalls.md`　／　225 行
