-- 電力小売データ分析基盤：全 SQL 結合版（配備順：テーブル定義 → 関数 → ビュー → プロシージャ）
-- 注意：${GCP_PROJECT_ID} / ${BQ_DATASET_*} はテンプレート変数。CI/CD で展開してから dry run し、
--       テスト用データセットへ適用してから本番へ配備する（命名・運用定義書 3.2 / 11.1）。
-- 本ファイルは 06_ソースコード/sql の各ファイルを配備順に結合したもの。修正は各ファイルに対して行う。
...
-- =============================================================================
-- 電力小売データ分析基盤
-- Gold 層 マート（agg_*・snap_*）DDL
-- 根拠：基本設計書 第9章（テーブル定義）・詳細設計書 第3部 1. 日報損益マート生成コアバッチのロジック詳細仕様、詳細設計書 第4部 3. データ品質（DQ）モニタリングマートと運用KPI
-- 注意：型・NOT NULL は設計書の記載に従う。
--       設計書に型の記載がない列は、命名規約から型を定めている。
--       デプロイ前に dev 環境で DDL レビュー（13.1／フェーズ1完了条件）を行うこと。
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS agg_daily_pnl (
  ...
  cost_capacity_contribution         NUMERIC NOT NULL OPTIONS(description = '原価内訳：容量拠出金：受電端kWh × 一律kWh単価'),
  ...
  contribution_margin                NUMERIC NOT NULL OPTIONS(description = 'コマ限界利益（利益階層①）：需要行：rev_energy − cost_jepx_spot − cost_jepx_fee − cost_procurement_contract、発電行：rev_market_sales − cost_jepx_fee。市場調達と小売／売電価格の純粋なスプレッド（10.14）'),
  gross_profit                       NUMERIC NOT NULL OPTIONS(description = '粗利（利益階層②＝③）：revenue − procurement_cost。賦課金は売上（rev_levy）と原価（cost_levy_passthrough）の双方に同額計上されるため相殺され、調整後売上総利益（②）と会計上の粗利（③）は同値になる（10.14）'),
...
CREATE TABLE IF NOT EXISTS agg_forecast_accuracy_daily (
  ...
  is_public_holiday                  BOOL NOT NULL OPTIONS(description = '休日属性：「公的休日＝0 かつ顧客休日＝0（登録なし）なのにバイアス率が大きなマイナス」なら顧客休日の登録漏れの疑い（15.4）'),
  is_customer_holiday                BOOL NOT NULL OPTIONS(description = '休日属性：「公的休日＝0 かつ顧客休日＝0（登録なし）なのにバイアス率が大きなマイナス」なら顧客休日の登録漏れの疑い（15.4）'),
...
-- 根拠：詳細設計書 第3部 3. コンソーシアムBG按分プロシージャのロジック詳細仕様
-- 注意：${GCP_PROJECT_ID} と ${BQ_DATASET_*} はテンプレート変数。CI/CD で環境別の値へ展開してから実行する
--       （命名・運用定義書 3.2）。未展開のまま実行してはならない。
...
-- 根拠：詳細設計書 第4部 3.2 祝日CSV取込のフェイルセーフ（検疫・アラート制御）
-- 注意：${GCP_PROJECT_ID} と ${BQ_DATASET_*} はテンプレート変数。CI/CD で環境別の値へ展開してから実行する
--       （命名・運用定義書 3.2）。未展開のまま実行してはならない。
