...
| 版 | 1.0（正式版） |
| 工程 | 04 基本設計 |
| 入力 | [機能一覧](../02_製品機能/機能一覧.md)、[ヒアリングシート兼適合分析表](../03_要件定義/ヒアリングシート兼適合分析表.md)、[知識体系](../01_基本知識/01_知識体系.md) |
| 作成・承認 | 作成：SE／承認：PM／レビュー：コンサルタント・管理者（業務要件）、運用担当（運用方針） |
...
> **本書の読み方**
> 第Ⅰ部で概念を固め、第Ⅱ部でデータモデルを定義し、第Ⅲ部で計算ロジックと運用方針を規定する。第Ⅳ部はオプション機能。
> 本書に登場する単価・損失率・制度仕様の数値は**設計イメージ**であり、導入時に一次情報（託送供給等約款、JEPX規程、OCCTO・資源エネルギー庁の公表資料）で確認して登録する。確認する項目はヒアリングシート兼適合分析表で管理し、本書では「ヒアリング Fnn-nn」の形で示す。
> 本書中の「命名・運用定義書」「規約」は、外部の [データ基盤開発運用ガイドライン](https://github.com/dpftriv/bigquery-dev-2nd-operation-guideline) を指す。
...
4. [アーキテクチャ（生データ / 中間層 / 最上位層）](#4-アーキテクチャbronze--silver--gold)
5. [実体関連図](#5-er図)
6. [テーブル一覧](#6-テーブル一覧)
7. [中間層 マスタ定義](#7-silver層-マスタ定義dim_)
8. [中間層 ファクト定義](#8-silver層-ファクト定義fact_)
9. [最上位層 定義](#9-gold層-定義agg_--snap_--v_)

**第Ⅲ部 ロジックと運用**

10. [計算式・ロジック仕様](#10-計算式ロジック仕様)
11. [日報集計バッチ仕様](#11-日報集計バッチ仕様)
12. [取込・スケジューリング仕様](#12-etlスケジューリング仕様)
13. [バリデーション・クレンジング・データ品質](#13-バリデーションクレンジングデータ品質)
14. [分析画面設計と性能](#14-bilooker-studio設計と性能)
...
| 入力 | 本書での使い方 |
|---|---|
| [機能一覧](../02_製品機能/機能一覧.md) | 本書が設計する標準機能。下表で機能と本書の章を対応付ける |
| [ヒアリングシート兼適合分析表](../03_要件定義/ヒアリングシート兼適合分析表.md) | 導入先ごとの確認結果と GAP 対策。本書中の「ヒアリング Fnn-nn」は同表の項目を指す。GAP 対策で設計を変える場合は、該当機能の章を改訂する |
| [知識体系](../01_基本知識/01_知識体系.md) | 用語・時間・確定性・損益の考え方。各章の冒頭に関連する知識を示す |

| 機能ID | 機能 | 本書の主な章 |
|---|---|---|
| F-01 | [基盤マスタ・取引先管理](../02_製品機能/機能一覧.md#f-01-基盤マスタ取引先管理) | [7. 中間層 マスタ定義](#7-silver層-マスタ定義dim_)、[1.5 共通の前提](#15-共通の前提) |
| F-02 | [需要家・契約管理](../02_製品機能/機能一覧.md#f-02-需要家契約管理) | [7. 中間層 マスタ定義](#7-silver層-マスタ定義dim_)、[13.1 マスタ登録時のバリデーション](#131-マスタ登録時のバリデーション) |
| F-03 | [単価・制度マスタ管理](../02_製品機能/機能一覧.md#f-03-単価制度マスタ管理) | [7. 中間層 マスタ定義](#7-silver層-マスタ定義dim_)、[1.5 共通の前提](#15-共通の前提) |
| F-04 | [カレンダー・休日管理](../02_製品機能/機能一覧.md#f-04-カレンダー休日管理) | [10.5 休日判定](#105-休日判定)、[12.7 祝日一覧ファイルの取込仕様](#127-祝日csvsyukujitsucsvの取込仕様)、[12.8 カレンダーの生成と差分拡張](#128-カレンダーの生成と差分拡張p_maintain_calendar_master) |
| F-05 | [市場・外部データ取込](../02_製品機能/機能一覧.md#f-05-市場外部データ取込) | [8. 中間層 ファクト定義](#8-silver層-ファクト定義fact_)、[12.6 外部コードの名寄せ（生データ → 中間層）](#126-外部コードの名寄せbronze--silver) |
| F-06 | [計画管理](../02_製品機能/機能一覧.md#f-06-計画管理) | [8. 中間層 ファクト定義](#8-silver層-ファクト定義fact_)、[10.3 インバランス量と需給調整グループ按分](#103-インバランス量とbg按分) |
| F-07 | [実績管理（3層）](../02_製品機能/機能一覧.md#f-07-実績管理3層) | [4.3 実績データの3層分割（中間層 内部）](#43-実績データの3層分割silver-内部)、[8. 中間層 ファクト定義](#8-silver層-ファクト定義fact_)、[13.4 自動補完アルゴリズム（確報層）](#134-自動補完アルゴリズム確報層) |
| F-08 | [小売売上計算](../02_製品機能/機能一覧.md#f-08-小売売上計算) | [3. 電圧クラス別 料金メニューの構造](#3-電圧クラス別-料金メニューの構造)、[10.4 市場連動単価](#104-市場連動単価)、[10.6 固定単価メニューの単価判定](#106-固定単価メニューの単価判定)、[10.7 小売売上（需要家単位・コマ単位）](#107-小売売上需要家単位コマ単位) |
| F-09 | [実量制契約電力判定](../02_製品機能/機能一覧.md#f-09-実量制契約電力判定) | [10.11 最大需要電力（実量制）— 個別判定・グループ合算の両対応](#1011-最大需要電力実量制-個別判定グループ合算の両対応) |
| F-10 | [調達原価計算](../02_製品機能/機能一覧.md#f-10-調達原価計算) | [10.8 真の調達原価（コマ単位）](#108-真の調達原価コマ単位)、[10.2 送電損失補正](#102-送電損失補正) |
| F-11 | [インバランス算定・BG按分](../02_製品機能/機能一覧.md#f-11-インバランス算定bg按分) | [10.3 インバランス量と需給調整グループ按分](#103-インバランス量とbg按分) |
| F-12 | [発電収支（FIP・発電側課金）](../02_製品機能/機能一覧.md#f-12-発電収支fip発電側課金) | [10.9 市場連動買取 発電収支](#109-fip-発電収支)、[10.10 発電側課金](#1010-発電側課金) |
| F-13 | [日報損益（利益階層）](../02_製品機能/機能一覧.md#f-13-日報損益利益階層) | [11. 日報集計バッチ仕様](#11-日報集計バッチ仕様)、[10.14 利益階層と分析指標](#1014-利益階層と分析指標) |
| F-14 | [検算・品質ゲートと公開](../02_製品機能/機能一覧.md#f-14-検算品質ゲートと公開) | [11. 日報集計バッチ仕様](#11-日報集計バッチ仕様)、[14.10 データ抽出（抽出）連携と日報公開の同期](#1410-データ抽出extract連携と日報公開の同期) |
| F-15 | [月次確定・精算](../02_製品機能/機能一覧.md#f-15-月次確定精算) | [1.5 共通の前提](#15-共通の前提)、[11. 日報集計バッチ仕様](#11-日報集計バッチ仕様) |
| F-16 | [需要予測精度監査](../02_製品機能/機能一覧.md#f-16-需要予測精度監査) | [9. 最上位層 定義](#9-gold層-定義agg_--snap_--v_) |
| F-17 | [分析画面（BI）連携](../02_製品機能/機能一覧.md#f-17-分析画面bi連携) | [14. 分析画面設計と性能](#14-bilooker-studio設計と性能) |
| F-18 | [データ品質・運用監視](../02_製品機能/機能一覧.md#f-18-データ品質運用監視) | [13. バリデーション・クレンジング・データ品質](#13-バリデーションクレンジングデータ品質)、[15. 運用・ガバナンス](#15-運用ガバナンス) |
| F-19 | [データ保持・アーカイブ](../02_製品機能/機能一覧.md#f-19-データ保持アーカイブ) | [15. 運用・ガバナンス](#15-運用ガバナンス) |
| F-20 | [データ移行支援](../02_製品機能/機能一覧.md#f-20-データ移行支援) | [13.1 マスタ登録時のバリデーション](#131-マスタ登録時のバリデーション) |
| F-21 | [蓄電・揚水アセット損益](../02_製品機能/機能一覧.md#f-21-蓄電揚水アセット損益) | [16. オプション機能：蓄電・揚水アセット管理](#16-オプション機能蓄電揚水アセット管理) |
| N-01 | [基盤・データ構造](../02_製品機能/機能一覧.md#n-01-基盤データ構造) | [4. アーキテクチャ（生データ / 中間層 / 最上位層）](#4-アーキテクチャbronze--silver--gold)、[1.6 主要設計判断（サマリ）](#16-主要設計判断サマリ) |
| N-02 | [保持期間](../02_製品機能/機能一覧.md#n-02-保持期間) | [15. 運用・ガバナンス](#15-運用ガバナンス) |
| N-03 | [個人情報・アクセス制御](../02_製品機能/機能一覧.md#n-03-個人情報アクセス制御) | [15. 運用・ガバナンス](#15-運用ガバナンス) |
| N-04 | [環境分離・配備](../02_製品機能/機能一覧.md#n-04-環境分離配備) | [15. 運用・ガバナンス](#15-運用ガバナンス)、[6. テーブル一覧](#6-テーブル一覧) |
| N-05 | [性能・コスト](../02_製品機能/機能一覧.md#n-05-性能コスト) | [14. 分析画面設計と性能](#14-bilooker-studio設計と性能) |
| N-06 | [命名・開発規約](../02_製品機能/機能一覧.md#n-06-命名開発規約) | [1.3 命名規約](#13-命名規約) |
...
> **対応機能**：[基盤・データ構造](../02_製品機能/機能一覧.md#n-01-基盤データ構造)、[命名・開発規約](../02_製品機能/機能一覧.md#n-06-命名開発規約)　／　**関連知識**：[知識体系 1.1 共通設計原則](../01_基本知識/01_知識体系.md#11-共通設計原則)、[講義 応用 第1章](../01_基本知識/05_講義_応用_実務の落とし穴.md#第1章計算順序の罠1円が合わない理由)
...
| `FT-nn` | Silver ファクト（計画・実績・監査） | `Fnn-nn` | ヒアリング項目（ヒアリングシート兼適合分析表） |
...
| 税 | DWH 内部は**原則税抜**で統一し、請求時にのみ課税する。**例外は再エネ賦課金のみ**（税込のまま保持・計算し、**日次・コマ段階では税込総額を保持**、月次精算・請求突合の段階で一括して丸め・税抜化する。10.7）。ソース別の保持ルールは下表で固定する |
...
|---|---|---|---|
| JEPXスポット・時間前価格 | 税抜 | **税抜（生の価格をそのまま）** | 仕入原価の基準 |
| インバランス料金 | 税抜 | **税抜（そのまま）** | 公的精算の基準 |
...
**計算順序**：DWH 内部では単価を足し合わせた「合成単価」を作らず、**費目ごとに `Σ(電力量 × 単価)` の総額を別カラムで持つ**（市場・手数料・託送・容量拠出金・賦課金…）。丸めは総額に対して、請求元の約款が定める方式（`dim_settlement_rounding_rules`, D-32）で**月次精算時に1回だけ**行う。コマ単位・単価単位での丸めは禁止する。理由：JEPX や一送の請求は「月間電力量 × 単価 → 丸め」または「コマ約定額 → 税込丸め」であり、DWH がコマごと・単価ごとに丸めると月次突合（検算 #2）で必ず数円ズレる。

| 託送料金（従量・基本） | 税抜 | **税抜（そのまま）** | 送配電事業者の請求ベース |
| 容量拠出金単価 | 税抜 | **税抜（そのまま）** | 公的負担金ベース |
| JEPX手数料 | 税抜 | **税抜（そのまま）** | JEPX 取引規程の表示どおり |
...
本基盤の主要な設計判断を一覧にする。各項目の詳細は参照先の章・節にある。
...
| 11 | **FIP・バランシングコスト** | A値は GIO 確定値を翌月洗替（日次は暫定）。バランシングコストは認定年度×実需給年度×電源×エリアの4軸マスタで持ち、激変緩和上乗せの終了年度以降は `subsidy_premium=0` を明示登録して自動消失させる（年度・単価は公表値、ヒアリング F03-04） | D-15／FX-05／10.9 |
...
> **関連知識**：[知識体系 3章 事業・業務の基本モデル](../01_基本知識/01_知識体系.md#3-事業業務の基本モデル)、[講義 初級](../01_基本知識/02_講義_初級_業界基礎知識.md)
...
> **対応機能**：[小売売上計算](../02_製品機能/機能一覧.md#f-08-小売売上計算)、[実量制契約電力判定](../02_製品機能/機能一覧.md#f-09-実量制契約電力判定)　／　**関連知識**：[知識体系 4章 契約・約款・料金・単価](../01_基本知識/01_知識体系.md#4-契約約款料金単価)
...
- 顧客ごとの相対条件（個別の手数料率、価格キャップ、最低保証など）はメニューコードでは表現しきれないため、`dim_market_linked_parameters` を `rate_menu_code × area_code` で持ち、顧客固有条件は専用の特約テーブルで扱う。
...
> **対応機能**：[基盤・データ構造](../02_製品機能/機能一覧.md#n-01-基盤データ構造)　／　**関連知識**：[知識体系 8.3 生データから人工知能までのレイヤー](../01_基本知識/01_知識体系.md#83-rawからaiまでのレイヤー)、[講義 応用 第2章](../01_基本知識/05_講義_応用_実務の落とし穴.md#第2章時間と鮮度の罠)
...
| **Silver** | 型変換・クレンジング・名寄せ済みの**正規化モデル**。マスタ（`dim_`）とファクト（`fact_`）、事実の統合ビュー（`v_`） | `dim_` / `fact_` / `v_` | 制約・バリデーション適用済み | アナリスト、バッチ、アプリケーション |
| **Gold** | 業務目的に沿って**集計・結合済み**の層。日報ダッシュボードが直接参照する | `agg_` / `snap_` / `v_` | 業務ロジック適用済み | 経営、トレーダー、オペレーター、BI |
...
│ SILVER                                                             │
│   dim_*  マスタ（全て期間管理）                                        │
│   fact_*  ファクト（30分コマ粒度。実績は stream / daily / settled の3層） │
│   v_*  事実の統合ビュー（3層の実績を1本の時系列に）                    │
└───────────────┬──────────────────────────────────────────────────┘
                │ 結合・按分・損益計算（第10章の式、第11章のバッチ）
┌───────────────▼──────────────────────────────────────────────────┐
│ GOLD                                                               │
│   agg_*   集計マート（agg_daily_pnl, agg_imbalance_daily, agg_monthly_summary）│
│   agg_*  マテリアライズドビュー（追記のみの単純集約に限定）              │
...
[① 速報ストリーム]       [② 日報（確報）]        [③ 月次確定]
 fact_*_actuals_stream      fact_*_actuals_daily       fact_*_actuals_settled
...
上記は `daily` に無いコマだけを速報で補うため、06:00 以降は確報値に自然に切り替わる（同一コマの二重計上は起きない）。この方式が成立する前提として、`fact_*_actuals_stream_latest` の保持期間を**当日のみから直近7日**へ延長する（FT-13／15.1）。`NOT EXISTS` の相関サブクエリは `target_date` で両側をパーティション絞り込みしているため、直近7日 × 地点 × 48 コマの範囲でしか走らない。

> **ビューの中で速報ストリームにウィンドウ関数を掛けない。** 速報は地点数×48コマ×再送で日に数千万〜数億行になる。BIを開くたびに `ROW_NUMBER` で全体をソートすると表示が止まり、スキャン課金が跳ね上がる。最新行の抽出は**数分間隔のマイクロバッチ**で `fact_*_actuals_stream_latest`（地点×日×コマで1行、UPSERT）に書き出し、ビューはそれを読むだけにする。ストリーム本体は `inserted_at` の日付でパーティションし、マイクロバッチ側の抽出条件に `inserted_at >= TIMESTAMP_SUB(CURRENT_TIMESTAMP, INTERVAL 2 HOUR)` のような**静的な絞り込み**を必ず入れる。
...
当日 随時   速報取込         ──▶ fact_*_actuals_stream ──▶ agg_stream_monitor
翌日 06:00  確報取込         ──▶ ts_meter_raw ──▶ クレンジング ──▶ fact_*_actuals_daily
翌日 07:00  日報バッチ       ──▶ agg_daily_pnl / agg_imbalance_daily / agg_slot_summary_active
翌月 21日   確定取込         ──▶ fact_*_actuals_settled ──▶ 月次リラン ──▶ agg_monthly_summary
```
...
> **対応機能**：[基盤・データ構造](../02_製品機能/機能一覧.md#n-01-基盤データ構造)　／　**関連知識**：[知識体系 8.1 最低限の横断マスタ](../01_基本知識/01_知識体系.md#81-最低限の横断マスタ)
...
> 期間管理（SCD Type 2）される6マスタ（`dim_balancing_groups`／`dim_bg_members`／`dim_gen_supply_points`／`dim_customer_contracts`／`dim_rate_menus`／`dim_procurement_contracts`）は、**主キーの先頭列（業務上の識別子）を改定時にも変更しない複合キー**（識別子 + `start_date`）とする（13.1 ⑮）。また、`dim_customer_contracts` は `dim_procurement_contracts` への専属調達契約リレーション（`procurement_contract_id`）を持つ（10.8.4）。
...
> **図を読むうえでの補足**：(1) 取引先マスタ→需要家マスタは法人・個人とも必須参照（個人も取引先を登録し `entity_type='INDIVIDUAL'` で区別）のため `||--o{`。(2) 料金メニューマスタ→非化石証書価値マスタは外部キーではなく `env_value_type = certificate_type` の値域一致による結合。(3) 日報損益マートのセグメント・メニュー種別には蓄電（第16章。初期導入の対象外）と非FIPを含む。(4) 図には需要家契約履歴の契約アンペア、需要家マスタの親需要地点番号、精算明細の受領時税区分、検針サイクルの検針区分を含む。

Visio 版の ER 図（物理・論理の2ページ）は同じフォルダの `ER図.vsdx` にある。

---

## 6. テーブル一覧

> **対応機能**：[基盤・データ構造](../02_製品機能/機能一覧.md#n-01-基盤データ構造)、[命名・開発規約](../02_製品機能/機能一覧.md#n-06-命名開発規約)　／　**関連知識**：[知識体系 8.2 ファクトと粒度](../01_基本知識/01_知識体系.md#82-ファクトと粒度)
...
| Silver | `prod_silver` | `dim_*`／`fact_*`／Silver の `v_*`／UDF | マスタとファクトを同一データセットに置く（`prod_master` のような別名は使わない） |
| Gold | `prod_mart` | `agg_*`／`agg_*`／Gold の `v_*`／プロシージャ | BI 抽出用ビューもここ |
| ステージング | `prod_staging` | 移行・取込の一時表 | 検証ゲート通過後に Silver へ MERGE |

dev／stg は接頭辞を `dev_`／`stg_` に置き換える。SQL ファイルはデータセット修飾を持たず、デプロイスクリプトが環境ごとに付与する。
...
| V-10 | 利益階層日次ビュー | `v_profit_layers_daily` | – | 日×エリア×BG で流向を合算し、利益階層①②③と kWh あたり限界利益を返す（10.14） |

---

## 7. Silver層 マスタ定義（`dim_`）

> **対応機能**：[基盤マスタ・取引先管理](../02_製品機能/機能一覧.md#f-01-基盤マスタ取引先管理)、[需要家・契約管理](../02_製品機能/機能一覧.md#f-02-需要家契約管理)、[単価・制度マスタ管理](../02_製品機能/機能一覧.md#f-03-単価制度マスタ管理)、[カレンダー・休日管理](../02_製品機能/機能一覧.md#f-04-カレンダー休日管理)　／　**関連知識**：[知識体系 4.2 単価を独立した知識体系として管理する](../01_基本知識/01_知識体系.md#42-単価を独立した知識体系として管理する)、[知識体系 8.5 履歴管理が必要となるマスタ](../01_基本知識/01_知識体系.md#85-scd-type-2が必要となるマスタ契約データ)、[講義 中級 第4章](../01_基本知識/03_講義_中級_数字ができるまでを一日追う.md#第4章マスタとファクトと期間管理)

**DB基盤は Google Cloud BigQuery に固定する。** 本書のテーブル定義は BigQuery 標準SQL型（`STRING` / `INT64` / `NUMERIC` / `DATE` / `TIME` / `TIMESTAMP` / `JSON`）で記す。桁数・精度は型に含めず、必要な制約はバリデーション層（第13章）で担保する。参考として汎用表記との対応を示す。
...
**物理配置（全ファクト共通）**：`PARTITION BY target_date`（日次）、`CLUSTER BY area_code, supply_point_number`（需要側は `demand_point_number`）、`require_partition_filter = true`。詳細は 14.9。
...
| 一般送配電事業者コード | `ts_operator_code` | STRING | NOT NULL / UNIQUE | 託送請求データ・確報データのファイルヘッダーに入る送信元識別コード。**送配電網を保有する一般送配電事業者に付与されるコードであり、エリアに1つ**。**先頭ゼロを含む文字列として保持し数値化しない**。一括公開されていないため、EDI／ビジネスプロトコル仕様書の巻末マスタ、または受信済みファイルのヘッダーから抽出して登録する（ヒアリング F01-01） |
...
> `ts_operator_code` の実値は本書では確定していない。受信データのヘッダー等から抽出して投入する（ヒアリング F01-01）。
>
...
> **沖縄（`10`）は登録しない。** 業務範囲を北海道〜九州の9エリアに固定し、系統連系のない沖縄特有の市場ルールを設計から除外する。地点番号の先頭2桁が `10` の登録は拒否する（13.1 ③）。
...
> 実値は本書では確定していない。**A番号は一覧から一括投入できるが、5桁コードと一般送配電事業者コードは一括提供がなく、社内システム・過去の提出控え・受信データから抽出して作成する**（ヒアリング F01-01 に手順を記載）。
...
| **試運転中の売電計上区分** | `trial_revenue_treatment` | STRING | | `RECOGNIZE`（売電を売上計上する）／`EXCLUDE`（計上しない）。**認定内容・系統連系契約・約款によって異なるため地点ごとに設定する**（ヒアリング F12-01）。NULL は `EXCLUDE` とみなす |
...
| 市場売電売上 | `trial_revenue_treatment` で切り替える。`RECOGNIZE` なら通常どおり計上、`EXCLUDE` なら 0 として計上しない | 試運転中の売電可否は認定内容・連系契約で異なる（ヒアリング F12-01） |
| FIPプレミアム・バランシングコスト | **商業運転開始日以降に限定**。試運転期間は常に 0 | 交付対象は商業運転開始後という前提。運転開始前の発電にプレミアムを付けない |
| 発電側課金 | 起算日を `gen_charge_start_basis` で切り替える（D-34 参照）。既定は商業運転開始日 | 約款により連系日起算・COD起算が分かれる（ヒアリング F12-01） |
...
| **課金起算基準** | `gen_charge_start_basis` | STRING | NOT NULL | `COMMERCIAL_OPERATION`（商業運転開始日から課金）／`GRID_CONNECTION`（系統連系日＝試運転開始日から課金）。**約款により分かれるため一送ごとに登録する**（ヒアリング F12-01） |
| 割引率 | `discount_rate` | NUMERIC | NOT NULL | 0〜1。割引なしは 0 |
| 適用開始日／終了日 | `start_date` / `end_date` | DATE | NOT NULL | 制度改定に対応 |

**Fit & Gap**：割引の対象条件（既設電源の激変緩和、電源種別による差）と単価そのもの（`gen_charge_rate`／`gen_charge_kwh_rate`）は一次情報で確認する（ヒアリング F03-02）。
...
| **専属調達契約ID** | `procurement_contract_id` | STRING | FK → `dim_procurement_contracts` / NULL可 | 特定の相対・PPA契約（例：RE100顧客専用のコーポレートPPA）をこの需要家契約に**専属**させる場合に設定する。NULL の場合はエリア全体の調達（`dim_procurement_contracts` の紐付けなし分）から送電端需要比で按分される（10.8.4・13.1 ⑮） |
| 契約グループID | `contract_group_id` | STRING | INDEX | **実量制の判定単位を決めるキー**。`NULL`＝地点単体で最大kWを判定（個別判定）。値あり（例：`GRP_KAISHA_001`）＝同一IDを持つ全地点の同一コマ電力量を合算して判定（グループ実量制）。同一グループは同一 `account_id`・同一 `voltage_class` でなければならない（13.1 ⑪） |
...
| 按分方式 | `allocation_method` | STRING | NOT NULL | `SIMPLE_RATIO`／`FIXED_SHARE`／`CAUSER_PAYS`／`HYBRID`（10.3.2）。BG規約が唯一の正（ヒアリング F11-01） |
...
> 制度の根底は連帯責任だが、データ基盤にとっての論点は「その責任を**不平不満なく計算し、精算に落とし込む按分方式**」である。按分方式はBG規約で定められ、事業者間の契約そのものなので、**必ず規約の原文で確認する**（ヒアリング F11-01）。本書は方式ごとの計算ロジックを定義するもので、どれを採るかは決めない。
...
> 損失率は**エリア × 電圧**の2軸で決まる。送電距離が長いエリアほど高く、電圧が低いほど高い。具体値は各一般送配電事業者の約款が唯一の正（ヒアリング F03-01）。
...
| 休日昼間の単価規則 | `holiday_day_rate_rule` | STRING | NOT NULL | `STANDARD`（休日昼間はその他季昼間単価）／`SEASONAL`（休日昼間も季節単価を適用。夏季ピークは平日のみ）。一送約款の差異をこの列で吸収する（ヒアリング F03-02） |
...
| **適用スコープ** | `scope_level` | STRING | NOT NULL | `MENU`（メニュー×エリア：標準）／`CUSTOMER`（顧客単体）／`POINT`（地点単体）。現時点は `MENU` のみ運用し、特高の個別特約が確定した時点で `CUSTOMER` / `POINT` 行を追加する |
...
>
> **未対応の論点（ヒアリング F08-01）**：本マスタは**市場連動メニューのパラメータ**（調整率・手数料率・上下限）を対象とする。**固定単価メニューの需要家別値引き**（例：高圧固定プランAの平日昼間単価を特定顧客だけ 1 円引き）は、現状では顧客専用のメニューコードを D-08 に作る以外に表現手段がない。固定単価メニューにも特約を持たせる場合は、D-08 の単価列に対する上書き行（`dim_rate_menu_overrides`：`rate_menu_code × scope_level × 単価列ごとの差分`）を追加するか、D-25 に固定単価向けの上書き列を拡張する必要がある。
...
| 電力量一律拠出金単価 | `capacity_kwh_rate` | NUMERIC | NOT NULL | 円/kWh。**日報の調達原価に用いる単価** |
...
**設計方針**：容量拠出金は「30分コマごとの需要実績量（受電端 kWh）× 年度の一律単価（円/kWh）」を**そのまま**調達原価に算入する（10.8）。ピーク時kW按分のロジックは実装せず、10.12 は制度変更時の参考として残す。制度の見直しは年次で確認する（ヒアリング F03-03）。
...
| 公表単価（税込） | `fit_levy_rate_incl_tax` | NUMERIC | NOT NULL | 円/kWh・全国一律。**国の公表値をそのまま保持し、計算にもこの税込単価を使う**。単価を税抜化した列は持たない（端数ズレの原因になるため） |
...
> 検針期間は地点ごとに異なり（検針日が月内でばらつく）、同じ `target_date` でも請求月が地点により変わる。**日付だけで年度を切り替えるロジックを書かない。** 検針サイクルの入手方法（一送の検針データかスマートメーターの締め日か）はヒアリング F07-02 で確認する。
...
> 丸め方式の実値は各社の約款・JEPX 規程で確認して登録する（ヒアリング F15-01）。DWH のコマ粒度の金額は丸めず NUMERIC のまま保持し、**このマスタの方式で月次に丸めた結果と請求書を突合する**。差額は `settlement_rounding_adjustment` として Gold に受ける（1.5）。
...
- 年度別の単価は制度公表値に従い年次で登録する（ヒアリング F03-04）。
...
#### D-33 非化石証書価値マスタ `dim_nonfossil_certificate_prices`【期間管理】
...
> **料金用の休日判定は「自社の約款休日 → 公的休日（土日祝）」の順で判定する。** 自社の全社休日のうち `applies_to_tariff = 1` の日（年末年始・お盆・創立記念日など、供給約款で「当社が定める休日」と定めた日）は、曜日や祝日に関わらず最優先で休日単価の対象とする。登録がない日は `dim_public_holidays` で土日祝を判定する。**業務上の休業日（`applies_to_tariff = 0`）は料金に影響させない**（自社が休みでも顧客の電気は流れる）。用途ごとに参照先を固定する。
...
日報から年次決算まで同一の時間軸で集計するための基準。**初期構築は4月開始（4月〜翌3月）で行う。** ただし会計年度の開始月は将来変わり得るため、期間をハードコードせず**マスタ駆動**で持つ。
...
## 8. Silver層 ファクト定義（`fact_`）

> **対応機能**：[市場・外部データ取込](../02_製品機能/機能一覧.md#f-05-市場外部データ取込)、[計画管理](../02_製品機能/機能一覧.md#f-06-計画管理)、[実績管理（3層）](../02_製品機能/機能一覧.md#f-07-実績管理3層)　／　**関連知識**：[知識体系 5.3 データ状態と改訂番号](../01_基本知識/01_知識体系.md#53-データ状態と-revision)、[講義 応用 第2章](../01_基本知識/05_講義_応用_実務の落とし穴.md#第2章時間と鮮度の罠)

### 8.1 共通方針

- **物理配置**：全ファクト共通で `PARTITION BY target_date`（日次）、`CLUSTER BY area_code, supply_point_number`（需要側は `demand_point_number`）、`require_partition_filter = true`。BigQuery のパーティション列は1列のみのため、エリアはパーティションではなくクラスタで絞る。例外は次表のとおり。
- **数値型の統一**：金額・電力量・単価は全て `NUMERIC`（精度38桁・小数9桁）。`FLOAT64` は使わず、丸め誤差の発生を構造的に排除する。件数・フラグ・コマ番号は `INT64`。
...
| 税区分 | `tax_type` | STRING | NOT NULL | **`税抜` 固定**。逆算・丸めを行わない |
...
| 自社BG割当容量(kW) | `allocated_capacity_kw` | NUMERIC | | 割当方式は制度に依存（ヒアリング F05-01） |
...
#### FX-10 非化石証書購入 `fact_nonfossil_certificate_purchases`
...
## 9. Gold層 定義（`agg_` / `snap_` / `v_`）

> **対応機能**：[日報損益（利益階層）](../02_製品機能/機能一覧.md#f-13-日報損益利益階層)、[月次確定・精算](../02_製品機能/機能一覧.md#f-15-月次確定精算)、[需要予測精度監査](../02_製品機能/機能一覧.md#f-16-需要予測精度監査)、[データ保持・アーカイブ](../02_製品機能/機能一覧.md#f-19-データ保持アーカイブ)　／　**関連知識**：[知識体系 7章 指標体系](../01_基本知識/01_知識体系.md#7-kpi体系指標関数母集団分析軸)
...
| 原価内訳：容量拠出金 | `cost_capacity_contribution` | NUMERIC | NOT NULL | 受電端kWh × 一律kWh単価（ヒアリング F03-03） |
...
| **コマ限界利益（利益階層①）** | `contribution_margin` | NUMERIC | NOT NULL | 需要行：`rev_energy − cost_jepx_spot − cost_jepx_fee − cost_procurement_contract`、発電行：`rev_market_sales − cost_jepx_fee`。市場調達と小売／売電価格の純粋なスプレッド（10.14） |
| 粗利（利益階層②＝③） | `gross_profit` | NUMERIC | NOT NULL | `revenue − procurement_cost`。賦課金は売上（`rev_levy`）と原価（`cost_levy_passthrough`）の双方に同額計上されるため相殺され、調整後売上総利益（②）と会計上の粗利（③）は同値になる（10.14） |
...
#### T-03 コマ別集約マート `agg_slot_summary_active` / `agg_slot_summary_cold`
...
**日次アーカイブ（B-13：日報バッチ B-05 の末尾で実行。現用側の削除に DML DELETE は使わない）**
...
| 休日属性 | `is_public_holiday` / `is_customer_holiday` | INT64 | NOT NULL | **「公的休日＝0 かつ顧客休日＝0（登録なし）なのにバイアス率が大きなマイナス」なら顧客休日の登録漏れの疑い**（15.4・ヒアリング F04-01） |
...
**顧客休日の登録漏れ検知（15.4「予測精度悪化」の具体化・ヒアリング F04-01 のエスカレーション起点）**
...
> 予測アルゴリズムを疑う前に、営業・CS へ `dim_account_holidays` の更新漏れを確認する（B-10b／ヒアリング F04-01）。`is_customer_holiday = 1` で `expected_load_ratio` が登録されているのに外している場合は、モデルが特徴量を取り込めていない側の問題として切り分ける。
...
> 損益計算（履歴マスタの BETWEEN 結合、損失補正、手数料按分）は MV では表現しきれない。また日報バッチの「対象日パーティションを削除して再作成」する作りは MV の増分更新を無効化する。**損益を伴う Gold は `agg_*` としてバッチで物理生成する。**
...
| V-10 | 利益階層日次ビュー `v_profit_layers_daily` | 10.14 |
...
> **対応機能**：[小売売上計算](../02_製品機能/機能一覧.md#f-08-小売売上計算)、[実量制契約電力判定](../02_製品機能/機能一覧.md#f-09-実量制契約電力判定)、[調達原価計算](../02_製品機能/機能一覧.md#f-10-調達原価計算)、[インバランス算定・BG按分](../02_製品機能/機能一覧.md#f-11-インバランス算定bg按分)、[発電収支（FIP・発電側課金）](../02_製品機能/機能一覧.md#f-12-発電収支fip発電側課金)　／　**関連知識**：[講義 中級 第2章](../01_基本知識/03_講義_中級_数字ができるまでを一日追う.md#第2章1需要家1コマの粗利を手で組み立てる)、[講義 中級 第3章](../01_基本知識/03_講義_中級_数字ができるまでを一日追う.md#第3章按分という考え方)、[知識体系 6章 損益計算と利益ブリッジ](../01_基本知識/01_知識体系.md#6-損益計算と利益ブリッジ)
...
> **計画と実績の基準を揃えてから差分を取る。** 計画が送電端、実績が受電端のまま引くと、損失率分がそのままインバランスに化ける。逆に両方を受電端で引いて後から損失率を掛けると、11.3 検算 #1（電力量突合）で調達量（送電端）と合わなくなる。損失補正は**インバランス算出の直前に1回だけ**、送電端側に揃える方向で行う。一送が判定時に損失率を乗せる方式か、提出値が送電端かはヒアリング F06-01 で確認する。
...
```
コマごと：
  f_dem_actuals / f_gen_actuals（送電端）− fact_*_plans（GC時点）
...
メニュー固有の休日定義（土日祝か日祝のみか、年末年始の対象日）は `dim_holiday_rules` / `dim_holiday_rule_details` で表現する。上記ビューは標準ルール `STD` に相当し、メニュー別ルールとの連動はヒアリング F04-02 の結果で定める。
...
コマ単位・単価単位で丸めない原則（1.5 計算順序）は変わらない。変わるのは「税抜化の位置」だけであり、日次マートの `revenue` は参考値 `rev_levy` を含む税抜額として表示できる。
...
分母の「当月の総約定量」は月末まで確定しない。日次バッチで確定按分しようとすると、毎朝過去コマを再計算するか月末を待つかのどちらかになり、べき等性と速度を失う。**暫定単価は前月実績ベースを採用する。** 年間予算ベースや 0（期末一括）に比べ、直近の取引規模に最も連動した従量原価を毎日反映でき、日次粗利のブレが最も小さい。
...
エリア一括按分の対象は、いずれの需要家契約からも専属指定されていない契約のみに限定する
...
#### 10.9.1 発電側（電源・アグリゲーション事業）の費目一覧と利益
...
| 発電側課金 | 発電側も送配電網の維持費を負担する制度による費用。契約出力 × 発電側課金単価 × (1 − 割引率) の月割試算＋従量分（10.10） | 税抜 | 契約kW（従量分は送電端） |
...
② 調整後 売上総利益（発電側） ＝ ① ＋ FIPプレミアム（交付金）売上 ＋ バランシングコスト（交付金）売上 − 発電側課金
```
...
蓄電池の充電側の諸費用（16.7）は、本節の発電所の発電側課金とは別に扱う。本計算式の結果は日報損益マートの `cost_gen_charge` 列に計上する（11.5 STEP 3）。

### 10.11 最大需要電力（実量制）— 個別判定・グループ合算の両対応

高圧の実量制には、**地点ごとに独立して判定する約款**と、**1契約で複数地点を束ねて合算判定する約款（グループ実量制）**が混在する。両者を同一ロジックで扱うため、判定単位のキーを `contract_group_id` の有無で動的に切り替える。
...
> 3. **実績のない月も行として存在させてからウィンドウを掛ける。`RANGE` は値ベースなので `MAX` 自体は暦月12ヶ月の窓を保つが、`COUNT(*)` による新規契約判定（`months_in_window`）は欠損月を数えず誤判定し、`is_provisional` も欠損を検知できない。そのため、カレンダー骨格（`month_spine`）を左に置いて LEFT JOIN し、`ROWS BETWEEN 11 PRECEDING` で確実に12暦月を窓にする。欠損月は `missing_months_in_window` で可視化し、1以上なら 13.2 の欠測アラートに載せる。
...
グループ合算の場合、基本料金はグループ全体で1本算出し、地点ごとの内訳は**当月の地点別最大kWの比で傾斜按分する（標準実装）**。約款に特異な定めがある場合のみ調整カラムで吸収する。日次の日割試算（`agg_daily_pnl.rev_base_est`）は当月の最大kWが未確定のため地点数で等分し、月次確定（B-08）で最大kW比に置き換える。
...
| # | 利益レイヤー | 計算定義（構成要素） |
|---|---|---|
| ① | **コマ限界利益**（スプレッド／サヤ） | 従量料金売上 ＋ 市場売電売上<br>− JEPX市場調達原価 − JEPX従量手数料<br>− 相対・PPA調達原価 ± 先物・VPPA決済差金 |
| ② | **調整後 売上総利益**（事業粗利／ネット粗利） | ① ＋ 小売基本料金売上 ＋ FIPプレミアム（交付金）売上＋バランシングコスト（交付金）売上<br>− 託送料金（基本・従量） − 容量拠出金原価 − 非化石証書原価<br>− インバランス精算原価 − 発電側課金（発電行のみ） |
| ③ | **売上総利益**（会計上の粗利／グロス粗利） | ② ＋ 再エネ賦課金売上（参考税抜値） − 再エネ賦課金原価（同額） |
| ④ | **営業利益** | ③ − 販売費及び一般管理費（SGA） |
| ⑤ | **経常利益** | ④ ＋ 営業外収益 − 営業外費用 |

各階層の実務上の意味と部門責任は [知識体系 6.4 利益5階層と部門責任](../01_基本知識/01_知識体系.md#64-利益5階層と部門責任) を参照。
...
詳細な規定は [1.5 共通の前提](#15-共通の前提)（合成単価の禁止）、[10.2 送電損失補正](#102-送電損失補正)（送電端換算）、[10.7 小売売上](#107-小売売上需要家単位コマ単位)（再エネ賦課金の税込管理）による。

#### 10.14.3 需要側（小売事業）の費目一覧
...
> **対応機能**：[日報損益（利益階層）](../02_製品機能/機能一覧.md#f-13-日報損益利益階層)、[検算・品質ゲートと公開](../02_製品機能/機能一覧.md#f-14-検算品質ゲートと公開)、[月次確定・精算](../02_製品機能/機能一覧.md#f-15-月次確定精算)　／　**関連知識**：[講義 中級 第2章](../01_基本知識/03_講義_中級_数字ができるまでを一日追う.md#第2章1需要家1コマの粗利を手で組み立てる)、[講義 応用 第1章](../01_基本知識/05_講義_応用_実務の落とし穴.md#第1章計算順序の罠1円が合わない理由)
...
| 入力（Silver） | `fact_*_actuals_daily`, `fact_*_plans`, `fact_jepx_spot_prices`, `fact_jepx_trades`, `fact_imbalance_prices`, `fact_procurement_settlements`, 各 `dim_*` |
...
- 締め済み期間（`dim_fiscal_*.close_status = '確定'`）へのリランは承認必須。無断で走らせると決算数値が動く。

### 11.5 参照実装：日報損益マート生成プロシージャ `p_generate_daily_pnl`

11.2 の手順を BigQuery 標準SQL で示す参照実装。**本書の確定仕様（税抜JEPX、検針月基準の賦課金、一律kWh容量拠出金、定額手数料の暫定計上、料金用休日の自社休日優先、先物の営業日割戻し、FIP暫定A値）を全て織り込む。** DB基盤は BigQuery に固定しているため、方言の読み替えは不要。
...
BEGIN
  ...
  -- ───────────────────────────────────────────────────────────
  -- CALL p_generate_battery_pnl(p_target_date);   -- 第16章（初期導入ではペンディング。導入時にコメント解除）
...
| 確定仕様 | 反映箇所 |
|---|---|
| JEPX 税抜そのまま | `jepx.area_price` を逆算せず使用 |
| 賦課金は税込公表単価のまま検針月で解決し、**税込総額を正として保持** | `dim_meter_reading_cycles` → `dim_fit_levy_rates`。`rev_levy_incl_tax` に税込総額、`rev_levy` に税抜換算の参考値。税率は `dim_tax_rates`。`target_date` の BETWEEN も `1.1` のハードコードも使わない。丸め・税抜化は月次（B-08） |
...
| 容量拠出金は一律kWh（ヒアリング F03-03） | `demand_kwh × capacity_kwh_rate` |
| 定額手数料は前月実績ベースの暫定単価 × 当日約定量（10.8.1） | `fee_fixed_provisional` × `bp.bought_kwh` を `fixed_fee_provisional` に分離保持。月次確定（B-08）で差額を月末日に一括調整 |
| 料金用休日は自社の約款休日を最優先 | `rate_holiday` CTE。`applies_to_tariff = 1` の全社休日のみ |
| 先物は Base 全コマ／Peak は営業日の 17〜40 | `futures_adj` CTE。`FUTURES_PEAK` ルールで営業日判定 |
| FIP は `GREATEST(0, F−A)`、確定A値優先 | `a_value` CTE。`FINAL` がなければ `PROVISIONAL` |
...
> **対応機能**：[カレンダー・休日管理](../02_製品機能/機能一覧.md#f-04-カレンダー休日管理)、[市場・外部データ取込](../02_製品機能/機能一覧.md#f-05-市場外部データ取込)、[実績管理（3層）](../02_製品機能/機能一覧.md#f-07-実績管理3層)　／　**関連知識**：[講義 上級 第2章](../01_基本知識/04_講義_上級_設計判断と事業経済性.md#第2章運用統制)、[講義 応用 第2章](../01_基本知識/05_講義_応用_実務の落とし穴.md#第2章時間と鮮度の罠)
...
| 実績（速報） | スマートメーター／自社パルス／送配電API | 対象コマ直後〜随時 | `fact_*_actuals_stream` | 当日の需給監視 |
| 実績（確報） | 一般送配電ポータル／API | 翌日〜数日 | `fact_*_actuals_daily` | 翌朝バッチで日報生成 |
| 実績（確定） | 一般送配電 託送データ | 翌月20日頃 | `fact_*_actuals_settled` | 月次確定バッチで再計算 |
...
| 賦課金・託送改定 | 各社／国 | 年度・改定時 | `dim_*` | 承認フロー経由で反映 |
...
| B-04 | 前日確報取込 | 毎日 06:00 | 取得→クレンジング→`fact_*_actuals_daily` へ UPSERT | 実行 |
...
| B-05c | 蓄電池サイクル損益 ⭐ | 毎日 07:30 | `agg_battery_pnl` 生成（**初期導入の対象外。導入時に有効化**） | 実行 |
| B-06 | 先物取込 | 営業日 19:00 | 時価評価更新 | スキップ |
| B-07 | 月次確定取込 | **営業日カウンター起動**：検針日（または月末）から一送の N 営業日目（`dim_holiday_rules.TS_BUSINESS` で営業日を判定）に受領検知 → 取込 | `fact_*_actuals_settled` へ一括 INSERT、`fact_settlement_receipts` 更新。`expected_date` は営業日カウンターで算出 | 一送休業日はスキップし翌営業日に再判定 |
...
| B-10b | 取引先休日の棚卸し | 年1回＋都度 | 自社は総務確定後、顧客分は営業・CS経由で更新。**登録責任者・承認・遅延時のエスカレーションは業務プロセス側で定義する（ヒアリング F04-01）。システムは登録鮮度の監視（15.4）と `fact_master_change_log` への記録のみ担う** | – |
...
| 対象 | 方式 | 理由 |
|---|---|---|
| 実績確定層（`fact_*_actuals_settled`） | **打ち消し方式**：元レコードは変更せず、訂正差分を `record_type = 'CORRECTION'` のレコードとして新しい `batch_id` で追記（理由・承認者は FT-10 に記録）。参照側は地点×日×コマで合算する（SV-01／SV-02） | 請求・精算・決算の根拠となる計量値であり、監査上「いつ何が変わったか」を行として残す必要がある |
...
> 会計年度は**開始月をハードコードせず `dim_fiscal_years` から解決する**。祝日は `dim_public_holidays`、自社休日は `dim_account_holidays` から導出する。骨格に含まれる `power_season` の月定義（夏季 7〜9月、冬季 12〜2月）は設定値として外出しし、変更時は導出列と同様に差分更新する。
...
`p_refresh_calendar_derived_columns(dates)` は、渡された日付だけを対象に `dim_fiscal_*` / `dim_public_holidays` / `dim_account_holidays` を結合して導出列を `UPDATE` する。締め済み年度の日付が含まれる場合は承認フラグがなければスキップしてアラートする。
...
> **対応機能**：[需要家・契約管理](../02_製品機能/機能一覧.md#f-02-需要家契約管理)、[検算・品質ゲートと公開](../02_製品機能/機能一覧.md#f-14-検算品質ゲートと公開)、[データ品質・運用監視](../02_製品機能/機能一覧.md#f-18-データ品質運用監視)、[データ移行支援](../02_製品機能/機能一覧.md#f-20-データ移行支援)　／　**関連知識**：[知識体系 9.1 データ品質管理](../01_基本知識/01_知識体系.md#91-データ品質管理)、[講義 中級 第4章](../01_基本知識/03_講義_中級_数字ができるまでを一日追う.md#第4章マスタとファクトと期間管理)
...
> **対応機能**：[分析画面（BI）連携](../02_製品機能/機能一覧.md#f-17-分析画面bi連携)、[性能・コスト](../02_製品機能/機能一覧.md#n-05-性能コスト)　／　**関連知識**：[知識体系 10章 分析画面・人工知能・経営意思決定への接続](../01_基本知識/01_知識体系.md#10-biai経営意思決定への接続)、[講義 上級 第2章](../01_基本知識/04_講義_上級_設計判断と事業経済性.md#第2章運用統制)
...
利益階層の定義は [10.14 利益階層と分析指標](#1014-利益階層と分析指標)、禁則の背景と部門責任は [知識体系 6.4 利益5階層と部門責任](../01_基本知識/01_知識体系.md#64-利益5階層と部門責任) を参照。
...
- クラスタは左からのプレフィックスでしか効かない。**全ファクトで `(area_code, 地点番号)` の2列に固定**。電圧クラス等での絞り込みはマスタ結合で行う。列は最大4つまでだが、追加は性能測定で必要性が示された場合のみ。
...
| 検討点 | 判断 |
|---|---|
| 対応できる処理 | 単純な集約のみ。損益計算（BETWEEN結合・損失補正・手数料按分）は不可 → `agg_*` で物理生成 |
...
### 14.10 データ抽出（Extract）連携と日報公開の同期
...
Looker Studio の抽出データソースは、**設定画面のスケジュール自動更新（日次・指定時刻）**と手動更新をサポートする。バッチ完了イベントから API で抽出更新を直接キックする方式は、**Looker Studio の公開 API に抽出更新のエンドポイントが提供されているかを実装前に確認する**（ヒアリング F17-01）。`biengine` は BigQuery BI Engine の予約管理 API であり、Looker Studio の抽出更新とは別物。確認できるまでは次の方式で同期を担保する。
...
> **対応機能**：[データ品質・運用監視](../02_製品機能/機能一覧.md#f-18-データ品質運用監視)、[データ保持・アーカイブ](../02_製品機能/機能一覧.md#f-19-データ保持アーカイブ)、[保持期間](../02_製品機能/機能一覧.md#n-02-保持期間)、[個人情報・アクセス制御](../02_製品機能/機能一覧.md#n-03-個人情報アクセス制御)、[環境分離・配備](../02_製品機能/機能一覧.md#n-04-環境分離配備)　／　**関連知識**：[講義 上級 第2章](../01_基本知識/04_講義_上級_設計判断と事業経済性.md#第2章運用統制)、[知識体系 9.2 ガバナンスと変更管理](../01_基本知識/01_知識体系.md#92-ガバナンスと変更管理)
...
| Silver | **確定（`fact_*_actuals_settled`）** | **10年・本番環境上（Active）** | 監査・精算の原本。コールドストレージへ移さない |
| Silver | マスタ・監査ログ | 無期限（履歴を消さない） | – |
| Gold | `agg_slot_summary_active` | 直近2年 | 2年超は B-13 で `agg_slot_summary_cold` へパーティションコピーし、Active 側は `partition_expiration_days = 737` で自動削除（DML DELETE は使わない。T-03） |
...
- 監査・精算の原本（Silver 確定層）から経営マート（Gold）まで、**すべて本番の BigQuery 上に10年間保持**する。別ストレージへの退避・復元手順を持たないことで、過去の再計算（再処理、締め済み期間の承認付きリラン）を同じ SQL で実行できる。
...
| 環境 | 用途 | データ |
|---|---|---|
| dev | 開発・スキーマ変更 | サンプルデータ（初期はマスキングなし。閲覧制限の導入時に見直す） |
...
| 対象 | 制御 |
|---|---|
| `dim_dem_customers.pii_*` 列の個人情報 | **初期構築ではマスキングを行わない（生データ保持）。** 本番稼働までに個人情報取扱規程上の閲覧制限が必要になった場合に備え、BigQuery の **Policy Tags による列レベルアクセス制御（CLAC）** へ移行できるよう、対象列（氏名・住所・連絡先）を **`pii_` 接頭辞で物理的に分離**しておく（D-06）。移行時はスキーマ変更なしにタグ（例：`tag_restricted_pii`）の付与のみで制御でき、日報バッチ・BI は `pii_` 列を参照しないため影響を受けない |
...
| 顧客休日の登録漏れ疑い | `is_public_holiday = 0 AND is_customer_holiday = 0 AND bias_ratio < -0.30`（T-06 のクエリ） | 営業・CS（ヒアリング F04-01 のエスカレーション起点） |
| 取引先休日の登録鮮度 | `dim_account_holidays` の自社分が翌年度分未登録のまま年度末60日前を経過／主要顧客（上位N社）の翌年分が未登録のまま年末60日前を経過 | 総務・営業（**登録責任者とエスカレーション先は業務側で定義：ヒアリング F04-01**） |
...
> **対応機能**：[蓄電・揚水アセット損益](../02_製品機能/機能一覧.md#f-21-蓄電揚水アセット損益)　／　**関連知識**：[講義 上級 3.5 蓄電・揚水のサイクル損益](../01_基本知識/04_講義_上級_設計判断と事業経済性.md#-35-蓄電揚水のサイクル損益オプション導入時の考え方)
...
| **放電側 受給地点番号** | `discharge_point_number` | STRING | FK / INDEX | 放電＝売電。発電側として計量。**現時点は充電側と同一番号（1アセット＝1地点）を前提**。分離計量が必要になれば別番号を登録するだけで対応 |
...
| 総合往復効率(RTE) | `round_trip_efficiency` | NUMERIC | NOT NULL | **カタログスペックの公称値を固定参照**。損益計算はこの値のみで行い、実測値からの自動洗替（フィードバック）は行わない。更新は期間管理レコードの追加による手動のみ |
...
| 充電時拠出金免責フラグ | `charge_levy_exempt` | INT64 | NULL可 | **予備枠**。現時点では未使用。免責特例の発効時に 1 を設定し、V-03 の `FUTURE_OPTION` 分岐で参照する |
| BGコード（充電／放電） | `charge_bg_code` / `discharge_bg_code` | STRING | FK | 需要BG・発電BG双方に属し得る |
| 運開日 | `commissioning_date` | DATE | | |
| 適用開始日／終了日 | `start_date` / `end_date` | DATE | NOT NULL | 効率の見直し（メーカー保証値の改定など）は新レコード追加で対応。**FUTURE_OPTION**：実測効率による自動更新を導入する場合はここに `source='MEASURED'` 行を追加する設計とする |
...
| 損失量(kWh) | `loss_kwh` | NUMERIC | | **マスタの公称 RTE から算出**：`charge_kwh × (1 − RTE)`。FUTURE_OPTION：実測差（充電 − 放電 − SOC変動）への切替はここにアドオン |
| データステータス | `data_status` | STRING | NOT NULL | 速報／確報／確定 |

> `cycle_id` の採番ルール（例：SOC が下限から上昇を始めた時点で新規採番し、下限に戻るまでを1サイクル）を決める（ヒアリング F21-02）。これがないと1件の裁定取引の損益が追えない。
...
| 実効往復効率 | `actual_rte` | NUMERIC | **監視用の参考値**。マスタの公称値との乖離を見るためだけに保持し、損益計算には使わない。FUTURE_OPTION：実測効率を損益へ反映する場合はここから `dim_battery_assets` へ書き戻す |
...
物理ロス量の評価（蓄電残量の増減の控除）と往復効率の扱いは [16.3 蓄電・揚水アセットマスタ](#163-d-29-蓄電揚水アセットマスタ-dim_battery_assets期間管理)・[16.7 投資収益率集計ビュー](#167-roi集計ビュー実装ロジック) による。
...
| 前提 | 内容 |
|---|---|
| 地点の紐付け | `charge_point_number`（需要側）と `discharge_point_number`（発電側）で対応付ける。**現時点は1アセット＝1地点（両者が同一番号）を前提とし、分離計量や一送ごとの計量ルールは組み込まない**。分離計量が必要になった場合は両列に別番号を登録するだけで対応できる |
| **効率の扱い** | マスタの公称 RTE（固定値）で四則演算する。実測値からの動的洗替は実装しない。`actual_rte` は監視用の参考値 |
| 手数料マスタ | `trade_side` は `買`／`売` で1行ずつ登録する（結合時の行増殖防止） |
| 期間結合 | 損失率・託送・手数料・容量拠出金は全て `target_date BETWEEN start_date AND end_date` を付ける |
| 税 | 内部はすべて税抜 |
| **充電側の諸費用** | 充電は「系統からの買電（需要）」と同義。**通常の需要家と同じ託送電力量料金（`demand_variable_rate`）のみ**を原価に算入し、**容量拠出金・発電側課金は現時点では加算しない（0）**。制度が確定した場合の挿入位置を `FUTURE_OPTION` コメントで残す |
...
charge AS (    -- 充電側：需要として計量、調達原価が発生
  SELECT ba.asset_id, dem.target_date, dem.slot_number, dem.area_code,
         ...
         SUM(dem.actual_value_kwh *
             (  jepx.area_price / (1 - loss.loss_rate)   -- 損失補正後の市場価格（税抜）
              ...
              -- FUTURE_OPTION: 蓄電池充電に対する容量拠出金・発電側課金は現時点では加算しない（0）。
  ...
  -- FUTURE_OPTION: 拠出金を加算する場合は dim_capacity_contribution_rates を期間結合してここに追加
  GROUP BY 1,2,3,4
),
...
SELECT p.asset_id, ba.asset_name, ba.rated_capacity_kwh, ba.round_trip_efficiency,
       ...
       SAFE_DIVIDE(SUM(p.discharge_kwh),
                   SUM(p.charge_kwh) - (s.soc_close - s.soc_open) * ba.rated_capacity_kwh) AS actual_rte,  -- 監視用参考値。損益には ba.round_trip_efficiency（公称）を使う。FUTURE_OPTION
...
| サイクルの完結 | `cycle_id` 単位で充電と放電が対応。未完結は仕掛在庫として損益確定させない（ヒアリング F21-03） |
